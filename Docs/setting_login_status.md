...
Rezolve anonymously tracks shoppers. It doesn't need the shopper to be logged in to track them. The iOS library sets a unique ID for the device (called visitor ID) which stays with the app install for up to 1 year after the last activity. But, if the shopper is logged in, and the library is configured correctly, Rezolve also receives a second unique ID. This ID is for the shopper, not the device. This allows Rezolve to track the shopper across their devices if the Rezolve customer has configured multiple tracker SDKs (ex. web, Android, iOS) correctly.
