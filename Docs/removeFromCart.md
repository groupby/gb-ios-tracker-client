...
let event = RemoveFromCartEvent(cart: cart, googleAttributionToken: "abc123")

// Prepare beacon for request
let beacon = RemoveFromCartBeacon(event: event, experiments: nil, metadata: nil)

// Use tracker instance to send beacon
tracker.sendRemoveFromCartEvent(removeFromCartBeacon: beacon) { error in
...
| onSale | Whether the product was on sale when the shopper removed it from their cart. | `Bool` | Yes | n/a | n/a | n/a |
...
| Property | Description | Swift type | Required? | Min | Max | String format |
| -------- | ----------- | --------- | --------- | --- | --- | ------------- |
| items | The cart items being removed from the cart. | `[CartItem]` | Yes | 1 | 1000 | n/a |
