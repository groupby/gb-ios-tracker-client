# Rezolve Tracker Client for iOS

This is the iOS SDK used to send beacons to Rezolve.
...
Login status describes whether the shopper is logged in or not when the event occurs. With this information set in the tracker Rezolve can anonymously track shoppers across their devices, not just anonymously track them in the iOS app.
...
The following event types are supported in the client. The "main four" event types are what Rezolve considers to be a minimum required beacon implementation in your iOS app:
